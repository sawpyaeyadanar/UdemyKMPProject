...
@Composable
///   getViewModel() က koin ကနေ လာမှာ
fun ArticlesScreen(onSourcesButtonClick: () -> Unit, onAboutButtonClick: () -> Unit, articlesViewModel: ArticlesViewModel = koinViewModel()) {
    /* StateFlow's articlesState ရဲ့ state တွေကို collect လုပ် */
...
@Composable
private fun AppBar(onSourcesButtonClick: () -> Unit, onAboutButtonClick: () -> Unit) {
    TopAppBar(
...
fun ArticlesListView(viewModel: ArticlesViewModel) {
    SwipeRefresh(state = SwipeRefreshState(viewModel.articlesState.value.loading), onRefresh = { viewModel.getArticles(true) }) {
...
fun ArticleItemView(article: Article) {
    Column(modifier = Modifier
        ...
        Spacer(modifier = Modifier.height(4.dp))
        Text(text = article.date, style = TextStyle(color = Color.Gray), modifier = Modifier.align(
            Alignment.End
        ))
        Spacer(modifier = Modifier.height(4.dp))
    }
